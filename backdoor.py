...
def shell(command):  # функция, которая будет выполнять команды и возвращать результат
    ...
    # от коммандной строки и добавляем его в переменную, отвечающую за ответ и после чего возвращаем эту переменную
...
while True:
    ...
    # дешифруем их, обрезаем пробелы с начала и с конца и получаем ответ от консоли
